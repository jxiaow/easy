...
import static cn.xwj.easy.util.TouchUtil.isTouchPointInView;

/**
 * 带头部的ExpandableListView
 * <p>
 * Created by xwj on 2017/10/17.
 */
...
public class PinnedHeaderExpandableListView extends ExpandableListView
     ...
     *
     * @param context 上下文
     * @param resId   头部layout
     */
    public void setHeaderView(Context context, int resId) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (mHeaderViewVisible) {
            int x = (int) ev.getX();
            int y = (int) ev.getY();
            if (isTouchPointInView(mHeaderView, x, y)) {
                headerViewClick();
                return true;
            }
        }
    ...
    private void configureHeaderView(int groupPosition, int childPosition) {
        if (mHeaderView == null || mAdapter == null
                || (((ExpandableListAdapter) mAdapter).getGroupCount() == 0 && !mHeaderViewVisible)) {
            return;
        }
...
//    }
    ...
    public interface HeaderAdapter {
         ...
         */
        int getHeaderState(int groupPosition, int childPosition, boolean isGroupExpand);
